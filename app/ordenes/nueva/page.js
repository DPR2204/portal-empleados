...
export default function NuevaOrden() {
  ...
  const guardar = async e => {
    ...
    if (!r.ok) {
      setErr(j.error || 'Error al generar orden');
      return;
    }
  ...
  return (
    <main>
      ...
      <form onSubmit={guardar} style={{ display:'grid', gap:12, maxWidth:420 }}>
        <input
          name="colaboradorId"
          placeholder="ID público (ej. EMP-ABCDE)"
          value={campos.colaboradorId}
          onChange={cambia}
          required
        />
        ...
        {campos.frecuencia !== 'DIAS' ? (
          <>
            <input
              name="anio" type="number" placeholder="Año"
              value={campos.anio} onChange={cambia} required
            />
            <input
              name="mes" type="number" placeholder="Mes (1–12)"
              value={campos.mes} onChange={cambia} required
            />
          ...
          <>
            <input
              ...
              value={campos.inicio} onChange={cambia} required
            />
            <input
              name="fin" type="date"
              value={campos.fin} onChange={cambia} required
            />
            <input
              name="dias_laborados" type="number" placeholder="Días laborados"
              value={campos.dias_laborados} onChange={cambia} required
            />
          </>
        )}

        <input
          name="otros_descuentos" type="number" step="0.01"
          placeholder="Otros descuentos (opcional)"
          value={campos.otros_descuentos} onChange={cambia}
        />
